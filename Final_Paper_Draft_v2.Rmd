...
```
\begin{center}
Figure 10 - Decomposed US Covid Case Timeseries Data
\vspace{5mm} %5mm vertical space
\end{center}

This visualization makes evident that this a non-stationary process. This is validated by running an augmented Dickey-Fuller (adf) Test where the the alternative hypothesis is that the dataset is stationary.
...
The large p-value calculated in this test thus shows that the null hypothesis that the timeseries is non-stationary cannot be rejected..For ARIMA model to be valid, the non-stationary aspects of this dataset must thus be removed. This can be accomplished by simply taking the difference between each data point. A plot of the decomposed difference-stationarized case timeseries dataset and the corresponding results of the adf test shown below. This dataset now no longer has any long term trends and the p-value for the stationary hypothesi is >0.01 so it is significant.
...
```
\begin{center}
Figure 11 - Decomposition of Differenced Covid Case Data
\vspace{5mm} %5mm vertical space
\end{center}
...
The same model development process was repeated for the timeseries of US Covid-19 deaths. 
\begin{center}
\vspace{5mm} %5mm vertical space
\end{center}
```{r,echo=FALSE}
...
```
\begin{center}
Figure  - Decomposition of raw US Covid-19 Death Data
\vspace{5mm} %5mm vertical space
\end{center}
```{r}
# Test for Stationarity
...
```
\begin{center}
Figure  - Decomposition of differenced US Covid-19 Death Data
\vspace{5mm} %5mm vertical space
\end{center}
```{r}
...
A key difference between the ARIMA models for cases and deaths are the optimal parameters for the non-seasonal and seasonal trends. For the death arima model, the parameters were calculated to be 1,0,2 and 0,1,1 for the non-seasonal and seasonal terms respectively.
```{r,echo=FALSE, results="hide",fig.align='center'}
## Check residuals
checkresiduals(fitARIMA_deaths)
```
The residuals in this model too are normally distributed around 0, so the model is unbiased.
...
```
\begin{center}
Figure 12 - 30 Day forecast for Covid-19 cases in US using AR Lag-7 Model. Dark blue indicates model results, light blue bars indicate 95 percent confidence interval.
\vspace{5mm} %5mm vertical space
\end{center}
...
```
\begin{center}
Figure  - 30 Day forecast for Covid-19 deaths in US using AR Lag-7 Model. Dark blue indicates model results, light blue bars indicate 95 percent confidence interval.
\vspace{5mm} %5mm vertical space
\end{center}
